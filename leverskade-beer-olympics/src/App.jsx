...
// Reads the address bar so the QR on the Beer Olympics pass can land people
// straight on the rules: .../#beer/rules
function readHash() {
  const raw = (window.location.hash || "").replace(/^#\/?/, "");
  const [sec, sub] = raw.split("/");
  const sections = ["info", "planning", "beer"];
  const tabs = ["leaderboard", "log", "beerpong", "roster", "info"];
  // The Beer Olympics passes were printed with a QR pointing at the bare URL,
  // so no hash means "straight to the Beer Olympics rules". #home is the
  // welcome page, reachable from the back link.
  if (sec === "home") return { section: "home", tab: "info" };
  if (!sections.includes(sec)) return { section: "beer", tab: "info" };
  return { section: sec, tab: sec === "beer" && tabs.includes(sub) ? sub : "info" };
}

export default function LeverskadeApp() {
  const [state, setState] = useState(null);
  // "home" | "info" (general) | "planning" | "beer"
  const [section, setSection] = useState(() => readHash().section);
  ...
  // Deep links: #beer/rules (what the Beer Olympics pass QR points at),
  // #planning, #info. Also keeps the address bar in step as you navigate.
  useEffect(() => {
    const onHash = () => {
      const h = readHash();
  ...
  useEffect(() => {
    const want = section === "beer" ? `#beer/${tab}` : `#${section}`;
  ...
  if (section === "home") return <Home go={setSection} />;

  if (section === "info") {
    return (
      <SectionShell title="General Info" emoji="📋" back={() => setSection("home")}>
        <GeneralInfo go={setSection} />
      </SectionShell>
    );
  }
  ...
  if (section === "planning") {
    return (
      <SectionShell title="Planning" emoji="📅" back={() => setSection("home")}>
        <Planning />
      </SectionShell>
    );
  }
  ...
  const cards = [
    {
      ...
      emoji: "📋",
      title: "General Info",
      body: "Crew, costumes, who brings what, what's included and how to stay warm & tick-free.",
    },
    {
      id: "planning",
      emoji: "📅",
      title: "Planning",
      body: "The full weekend schedule, timings and your personal packing list.",
    },
    {
      id: "beer",
      emoji: "🍺",
      title: "Beer Olympics",
      body: "Rules, live scoreboard, log your wins and follow the Beer Pong tournament.",
    },
  ];

  return (
    <div
      style={{ background: BRAND.cream, fontFamily: "'Space Grotesk', sans-serif" }}
      className="min-h-screen text-[#233020]"
    >
      <style>{`
      ...
      <header
        ...
        className="px-5 pt-12 pb-10 rounded-b-3xl relative overflow-hidden text-center"
      >
        <Mountain
          size={190}
          strokeWidth={1}
          className="absolute -right-10 -bottom-14 text-white opacity-10"
        />
        <div className="relative">
          <div
            style={{ fontFamily: "'Baloo 2', sans-serif" }}
            className="text-[11px] uppercase tracking-[3px] text-white/80 font-bold"
          >
            Welcome to
          </div>
          <h1
            style={{ fontFamily: "'Baloo 2', sans-serif" }}
            className="text-white text-[38px] leading-none font-extrabold tracking-tight mt-1"
          >
            LEVERSKADE
          </h1>
          <div
            style={{ fontFamily: "'Baloo 2', sans-serif", color: BRAND.orange }}
            className="text-[22px] font-extrabold leading-none mt-1"
          >
            2026
          </div>
          <p className="text-white/90 text-[13.5px] font-semibold mt-3 m-0">
            3–6 September · Jugendzeltplatz Monschau
          </p>
        </div>
      </header>

      <main className="px-4 pt-6 pb-10 max-w-md mx-auto space-y-3">
        {cards.map((c) => (
          <button
            key={c.id}
            onClick={() => go(c.id)}
            style={{ border: `1.5px solid ${BRAND.mint}` }}
            className="w-full text-left bg-white rounded-2xl p-4 shadow-[0_2px_10px_rgba(15,122,56,0.06)] active:scale-[0.99] transition-transform"
          >
            <div className="flex items-center gap-3">
              <span className="text-[26px] leading-none flex-none">{c.emoji}</span>
              <div className="min-w-0">
                ...
                <p className="text-[12.5px] text-[#6a7166] m-0 mt-0.5 leading-snug">{c.body}</p>
              </div>
              <ChevronRight size={20} className="ml-auto flex-none" style={{ color: BRAND.mint }} />
            </div>
          </button>
        ))}

        <p className="text-center text-[11.5px] text-[#a9b0a3] pt-3">
          🏕 Four days · One campsite · Questionable decisions
        </p>
      </main>
    </div>
  );
...
// Wrapper used by the two read-only sections (no bottom tab bar needed).
function SectionShell({ title, emoji, back, children }) {
  return (
    <div
      style={{ background: BRAND.cream, fontFamily: "'Space Grotesk', sans-serif" }}
      className="min-h-screen text-[#233020] pb-10"
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Baloo+2:wght@600;700;800&family=Space+Grotesk:wght@400;500;600;700&display=swap');
      `}</style>
      <header
        style={{ background: `linear-gradient(135deg, ${BRAND.green}, ${BRAND.greenDark})` }}
        className="px-5 pt-5 pb-5 rounded-b-3xl relative overflow-hidden"
      >
        <button
          onClick={back}
          className="flex items-center gap-1 text-white/85 text-[12.5px] font-semibold mb-1.5"
        >
          <ChevronLeft size={15} /> Leverskade
        </button>
        <h1
          style={{ fontFamily: "'Baloo 2', sans-serif" }}
          className="text-white text-2xl font-extrabold tracking-tight relative"
        >
          {emoji} {title}
        </h1>
      </header>
...
function Shell({ tab, setTab, syncing, offline, back, children }) {
  const tabs = [
    ...
    <div style={{ background: BRAND.cream, fontFamily: "'Space Grotesk', sans-serif" }} className="min-h-screen text-[#233020] pb-24">
      ...
      >
        ...
        <div className="flex items-center gap-2 relative">
          <button
            onClick={back}
            className="flex items-center gap-1 text-white/85 text-[12.5px] font-semibold"
          >
            <ChevronLeft size={15} /> Leverskade
          </button>
          {offline ? (
...
// Module level for the same reason as InfoCard: keeps its identity stable so
// the bracket doesn't remount on every background refresh.
function Match({ m, dim, nameOf, dispatch }) {
  if (m.isBye) {
    return (
    ...
    <div className="space-y-5">
      ...
      {phase === "group" && currentRound && (
        <div>
          ...
          <div className="space-y-2">
            {currentRound.map((m) => <Match key={m.id} m={m} nameOf={nameOf} dispatch={dispatch} />)}
          </div>
        </div>
      )}
      ...
      {phase === "knockout" && koRounds.map((round, i) => (
        <div key={i}>
          ...
          <div className="space-y-2">
            {round.map((m) => <Match key={m.id} m={m} dim={i < koCurrentIdx} nameOf={nameOf} dispatch={dispatch} />)}
          </div>
        </div>
      ))}
            ...
            <div className="space-y-4 mt-3">
              {rounds.slice(0, phase === "group" ? -1 : undefined).map((round, i) => (
                <div key={i}>
                  ...
                  <div className="space-y-2">
                    {round.map((m) => <Match key={m.id} m={m} dim nameOf={nameOf} dispatch={dispatch} />)}
                  </div>
                </div>
              ))}
            </div>
...
const GAME_RULES = {
  ...
  "Rage Cage Battle": {
    ...
    how: [
      ...
      "Sink it on your very first try and you hand the cup to anyone you like — as long as they don't already have one.",
      "Sink it on any later try and the cup goes to the player on your left, clockwise.",
      "Bounce the ball into another cup by accident and you drink that one, add it to the cup you're playing with, and only then carry on.",
      "Catch up with the player to your right and stack your cup inside theirs — they now have both.",
...
// Declared at module level on purpose: defining these inside Info would create
// new component types on every render, remounting the page and throwing the
// reader back to the top whenever a game is opened.
